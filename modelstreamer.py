...
#------------------------------------------------------------------->
class ModelStreamer:
	def __init__(self,symbol,streamer,filename=None,logpath=None,train_model=True,**kwargs):
		self.symbol = symbol
		...
		self.parameters['build_once'] = True
		self.parameters['timed'] = True
		self.parameters['impatient'] = False
		self.parameters['train_type'] = 'stream'
		...
		# MISC
		self.message_keys = ['id','ts','lastClose','open','high','low','close','volume','symbol']
		self.sequence = 0
		...
		print(f'Model: {self.filename} ready!')
	...
	def process_df(self,data,**kwargs):
		excludes = ['id']
		data = {self.message_keys[i]:data[i] for i in range(1,len(self.message_keys))}
		columns = self.uniques(self.features + [key for key in self.message_keys if key not in excludes])
		ts = datetime.fromtimestamp(data['ts'])
		data = self.get_indicators(mode='process',data=data,**kwargs)
		self.df.loc[ts,columns] = [data[column] for column in columns if column not in excludes]
		return self.df
	...
	def insert_row(self,data,**kwargs):
		...
		df2['move'] = 0
		print(df2)
		self.df.loc[self.df.index[-2],'move'] = 1 if self.df.loc[self.df.index[-2],'close'] - self.df.loc[self.df.index[-1],'close'] < 0 else 0 
		self.predictions.loc[self.predictions.index[-1],'actual'] = self.df.loc[self.df.index[-1],'move']
		if len(self.predictions) > 1:
			self.predictions.loc[self.predictions.index[-2],'actual'] = 1 if self.df.loc[self.df.index[-2],'close'] - self.df.loc[self.df.index[-1],'close'] < 0 else 0
	...
	def predict(self,minimum_positive=0.55):
		Y_ = self.model.predict(self.X_)[0][0]
		print(Y_)
		Y_ = 1 if Y_ > minimum_positive else 0

		predictions = pd.DataFrame(columns=['ts','actual', 'forecast'])
		if self.resolution == 'm':
			predictions['ts'] = pd.date_range(start=self.df.index[-1],
		...
		if self.producer is not None:
			print(self.df.tail())
			message = (datetime.timestamp(self.predictions.index[-1]), # Prediction Timestamp
					   Y_, # Prediction
					   self.df.loc[self.df.index[-1],'open'], # Open price
					   self.df.loc[self.df.index[-2],'close'] # Last close
			) 
			self.producer.send_prediction(message)
		print(self.predictions.tail())
		return Y_

	def get_predictions(self,minimum_positive=0.55):
		Y_ = self.model.predict(self.X_)[0][0]
		print(Y_)
		Y_ = 1 if Y_ > minimum_positive else 0
		...
		if self.resolution == 'm':
			self.predictions['ts'] = pd.date_range(start=self.df.index[-1],
												   periods=self.n_forecast,
												   freq="T")
		elif self.resolution == 'h':
			self.predictions['ts'] = pd.date_range(start=self.df.index[-1],
	...
	def insert_predictions(self,save=False):
		if len(self.predictions) < 2:
			return None
		ts = self.predictions.index[-2]
		ID = self.resolution+str(int(datetime.timestamp(ts)))+str(self.symbol)
		actual,pred = self.predictions.iloc[-2]
		close = self.df.loc[self.df.index[-2],'close']
		with Database() as db:
		...
		for key in kwargs:
			if key.lower() == 'maximize':
				...
				if type(kwargs[key]) == bool:
					...
					raise ValueError("impatient accepts a boolean value")
